...
def estrelas():
    ...
    for _ in range(num_estrelas):
        ...
        velocidade = random.randint(1, 5)  # Velocidade da estrela
    ...
    while rodando:
        ...
        # Preencher o fundo com a cor preta
                tela.fill((j*0.75,(40-j)*0.75,1.5*i), (20*j, 20*i, 20, 20))

        # Atualizar e desenhar as estrelas
        for estrela in estrelas:
            x, y, tamanho, velocidade = estrela
            y += velocidade  # Move as estrelas para baixo
            ...
            # Desenhar a estrela
            pygame.draw.circle(tela, (255, 255, 255), (x, y), tamanho)
...
def tela_inicial(): #função para desenhar a tela inicial
    ...
    for color in arco_iris:
        pygame.draw.rect(tela, color, (0, 0, largura, altura), 6, 1)  # 8 é a espessura
        pygame.display.update()
        time.sleep(0.1)
    pygame.display.update()
...
def pontuação_desenho(pontuação1, pontuação2):  #função para desenhar o placar
    ...
    texto_pontuação = fonte.render(f"{pontuação1} - {pontuação2}", True, azulferro)
...
def fundo_desenho(): #função para desenhar o fundo e as linhas
    ...
    for i in range(30):
        for j in range(40):
            pygame.draw.rect(tela, (j*0.75,(40-j)*0.75,1.5*i), (20*j, 20*i, 20, 20))
        pygame.draw.rect(tela, cinza, (largura // 2 - 2.5, i*20, 2.5, 15), 0)  #linha central


def loop_jogo():  #função para iniciar o jogo
    ...
    #config da musica
    pygame.mixer.music.load('C:/Users/Evelyn/Documents/FACULDADE/theme.mp3')
    ...
    while True:
        ...
        if bola_x - raio_bola <= player1_x + raquete_largura and player1_y < bola_y < player1_y + raquete_altura:
            ...
            bola_velocidade_y = (((bola_y - player1_y) / raquete_altura) - 0.5) * 10

        if bola_x + raio_bola >= player2_x and player2_y < bola_y < player2_y + raquete_altura:
            hit.play()
            bola_velocidade_x = -(bola_velocidade_x * (1 + dificuldade))
            bola_x = player2_x - raio_bola - 1
            bola_velocidade_y = (((bola_y - player2_y) / raquete_altura) - 0.5) * 10 
        ...
        if bola_x - raio_bola <= 0:
            ...
            dificuldade = 1.0  #resetando a dificuldade para o valor inicial
...
def timer_desenho(tempo_jogo):  #função para desenhar o timer na tela
    ...
    fonte = pygame.font.SysFont('verdana', 30)
    texto_tempo = fonte.render(tempo_formatado, True, roxosla)
...
pygame.mixer.init()
point = pygame.mixer.Sound('C:/Users/Evelyn/Documents/FACULDADE/point.wav')
hit = pygame.mixer.Sound('C:/Users/Evelyn/Documents/FACULDADE/hit.wav')
select = pygame.mixer.Sound('C:/Users/Evelyn/Documents/FACULDADE/select.wav')
